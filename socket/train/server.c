...
int main(int argc, char *argv[])
{
    ...
    if (-1 == listen(fd_lst, 10)) /* "man 2 listen" for detail information about argument 2 (backlog). in linux kernel 3.x, it doesn't care the backlog */
        goto error;
	...
	rfds_pre = rfds ;
        ...
        /* syscall : select */
        switch (select(FD_SETSIZE, &rfds_pre, NULL, NULL,NULL )) { /* exceptfds, monitors abnormal file handles */
            case 0: /* timeout */
            ...
            default: 
                if (FD_ISSET(fd_lst, &rfds_pre)) { /* here comes a connection */
                    ...
                    if (-1 == (fd_apt = accept(fd_lst, (struct sockaddr *)&raddr, &rlen)))
                        goto error;
        	   FD_SET(fd_apt, &rfds);//add fd_apt to array
			max_fd ++;
                }
		for ( fd_apt = 0 ;fd_apt < max_fd ;fd_apt++)
                if (FD_ISSET(fd_apt, &rfds_pre)) { /* data comes from client */
                    char buffer[4096] = {0};
                    /* recieve data, syscall : recv */
			printf("message arrive inner\n");
                    switch (recv(fd_apt, buffer, sizeof(buffer), MSG_DONTWAIT)) {
                        case 0: /* peer shutdown */
                            printf("peer shutdown\n");
			    close(fd_apt);
                            FD_CLR(fd_apt,&rfds); //clear 
                            fd_apt = -1;
                            break;
