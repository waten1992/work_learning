...
int main()
{
	...
	ts  test_arry[4] = {    };
	char char_arry[] = {'a','b','c','d'};
	for (int i = 0 ; i< 4 ; i++)
	{
		test_arry[i].int_type = i ;
		test_arry[i].char_type =char_arry[i]; 
 	}
	
	#endif 
	
	printf("ts size ->%d , test_arry--size--> %d \n",sizeof(ts),sizeof(test_arry));
